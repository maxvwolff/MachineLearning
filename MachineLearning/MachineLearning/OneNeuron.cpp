...
class Neuron {
	...
	public:
		...
		int evaluate(double, double);
		void learn(double, double, int);
};

Neuron::Neuron(double weight0, double weight1) {
	w0 = weight0;
	w1 = weight1;
}

int Neuron::evaluate(double x0, double x1) {
	return activate(x0 * w0 + x1 * w1);
}

void Neuron::learn(double x0, double x1, int correctLabel) {
	int error = correctLabel - evaluate(x0, x1);
...
int main() {
	...
	for (int i = 0; i < TESTING_SAMPLES; i++) {
		...
		int predictedLabel = neuron.evaluate(x0, x1);
		bool correct = false;
